...
public class MapNoRuleUnit {
	private String name;
	private HashMap<String, MapNoRuleLink> portToLink = new HashMap<String, MapNoRuleLink>();
	
	public MapNoRuleUnit(String nodeName) {
		this.name = nodeName;
	}
	...
	public void init(JSONObject connections) {
		for(Object connection: connections.keySet()) {
			...
			MapNoRuleLink link = new MapNoRuleLink(this.name, portName, nodeDst, portDst, band);
	...
	public MapNoRuleLink getLink(String port) {
	...
	public void search(String srcPort, Double band, String dstNode, ArrayList<String> nodesPassed
			, HashMap<String, MapNoRuleUnit> unitMap, ArrayList<ArrayList<String>> resultList) {
		...
		for(String portName: this.portToLink.keySet()) {
			if(!portName.equals(srcPort)) {
				if(this.portToLink.get(portName).getBand() > band) {
					...
					MapNoRuleUnit unit = unitMap.get(otherNode);
